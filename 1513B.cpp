...
{
    Long res = (ifact[r] % Mod * ifact[n - r] % Mod) % Mod;
    res = (res * fact[n]) % Mod;
    return res;
}
...
{
    if(n < r) return 0ll;
    Long ret = (1ll * fact[n] * inv[n - r]) % Mod;
    return ret;
}
int main(){
    solve();
    ...
    while(t--){
        int n;
        cin >> n;
        vector<int> v(n), tf(n), tr(n);
        for(int i = 0; i < n; i++){
            cin >> v[i];
            if(i == 0) tf[i] = v[i];
            else tf[i] = tf[i - 1] & v[i];
        }
        for(int i = n - 1; i >= 0; i--){
            if(i == n - 1) tr[i] = v[i];
            else tr[i] = tr[i + 1] & v[i];
        }
        Long ans = 0, res = 1;
        for(int i = 1; i < n; i++){
            res = (res * i) % Mod;
            if(tf[i - 1] == tr[i]){
                ans = (ans % Mod + (res % Mod * (n - i)) % Mod) % Mod;
                cout << res << " " << n - i << endl;
            }
        }
        cout << ans << endl;
