...
bool cmp(int i, int j, int x, int y){
    for(int k = i, h = x; k < j && h < y; k++, h++){
        if(a[k] != b[h]) return false;
    }
    return true;
}
int rec(int i, int j, int x, int y){
    ...
    ret = INT_MAX / 2;
    ret = min(ret, 1 + rec(i + 1, j, x, y));
    ret = min(ret, 1 + rec(i, j, x + 1, y));
    ret = min(ret, 1 + rec(i, j - 1, x, y));
    ret = min(ret, 1 + rec(i, j, x, y - 1));
    return ret;
}
int main(){
    FIO();
    ...
    while(t--){
        ...
        cin >> a;
        cin >> b;
        cout << rec(0, a.size(), 0, b.size()) << endl;
    }
    return 0;
}
