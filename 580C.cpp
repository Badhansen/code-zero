...
int dfs(int u, int cat){
    vis[u] = true;
    int sum = 0;
    if(cat > m) return 0;
    if((adj[u].size() == 1 && vis[adj[u][0]]) || adj[u].size() == 0) return 1;
    for(int i = 0; i < adj[u].size(); i++){
        int v = adj[u][i];
        if(!vis[v] && cats[v] == 0) sum += dfs(v, 0);
        else if(!vis[v] && cats[v]) sum += dfs(v, cat + 1);
    }
    return sum;
}
int main(){
