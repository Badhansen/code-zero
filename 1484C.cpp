...
/*
    ...
    not solve yet
*/
bool cmp(pair<int, vector<pair<int, int>>> &a, pair<int, vector<pair<int, int>>> &b){
    if(a.first == b.first){
        for(int i = 0; i < a.second.size() && i < b.second.size(); i++){
    ...
    while(t--){
        ...
        for(int i = 0; i < m; i++){
            ...
            for(int j = 0; j < days[i].size(); j++){
                temp.push_back({cnt[days[i][j]], days[i][j]});
            }
            //sort(temp.begin(), temp.end());
            ans.push_back({days[i].size(), temp});
        }
        //sort(ans.begin(), ans.end(), cmp);
        for(int i = 0; i < ans.size(); i++){
            cout << ans[i].first << " : ";
            for(int j = 0; j < ans[i].second.size(); j++){
                cout << ans[i].second[j].first << " "<< ans[i].second[j].second << endl;
            }
            cout << "---" << endl;
        }
    }
