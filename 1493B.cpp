...
int main(){
    ...
    while(t--){
        ...
        stringstream ss(src.substr(0, pos));
        ss >> hm;
        stringstream pp(src.substr(pos + 1));
        pp >> mm;
        int minute = hm * m + mm;
        int total = h * m + m;
        vector<pair<int, string> > dis;
        for(int i = 0; i < total; i++){
            int hour = (i / m);
            int minu = i % m;
            if(mirror(hour, h) != INT_MAX && mirror(minu, m) != INT_MAX){
                stringstream s;
                s << hour;
                string st, ans;
                s >> st;
                ...
                dis.push_back({min(total - minute, abs(minute - i)), ans});
            }
        }
        sort(dis.begin(), dis.end());
        for(auto el : dis){
            cout << el.fi << " " << el.ss << endl;
        }
        cout << dis[0].second << endl;
